...
-(void)setUpdataTitle:(NSArray *)updataTitle{
    _updataTitle = updataTitle;
    if (updataTitle.count == self.itemsTitle.count) {
        for (int i = 0; i < updataTitle.count; i ++) {
            FDSlideBarItem *item  = self.items[i];
            [item setTitle:updataTitle[i]];
