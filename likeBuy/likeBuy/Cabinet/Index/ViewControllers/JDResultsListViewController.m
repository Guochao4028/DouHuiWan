...
@interface JDResultsListViewController ()<GoodsListViewDelegate>
...
@implementation JDResultsListViewController
...
#pragma mark - 加载数据
-(void)loadDataJDGoodsList:(LoadType)type{
    ...
    JDCategoryModel *model = self.model;
...
-(void)refresh:(UITableView *)tableView{
    [self loadDataJDGoodsList:LoadTypeRefresh];
}

-(void)loadData:(UITableView *)tableView{
    [self loadDataJDGoodsList:LoadTypeMoreData];
}
...
-(void)setIndex:(NSInteger)index{
    _index = index;
    self.currentPage = @"1";
    [self loadDataJDGoodsList:LoadTypeStart];
}
